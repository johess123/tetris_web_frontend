...
const MultiPlayerGame = ({ user, roomData, onBack }) => {
    ...
    const [uiStates, setUiStates] = useState({ p1: p1Data.current, p2: p2Data.current });
    ...
    const drawNext = useCallback((ctxRef, typeIdx, level, size = 25) => {
        if (!ctxRef.current || typeIdx === undefined) return;
    ...
    const draw = useCallback(() => {
        const p1 = p1Data.current;
        const p2 = p2Data.current;
        ...
        drawBoard(canvasRef, p1);
        drawBoard(oppCanvasRef, p2);
    }, [drawCell]);
    ...
    const spawnBlock = useCallback(() => {
        ...
        if (collide(BLOCKS_ROTATIONS[g.blockType].rotations[g.rotationIndex], g.pos, g.grid)) {
            ...
            if (socket) {
                ...
                const otherUid = currentRoomStats.player_name.find(name => name !== user.account && name !== '');
                const otherData = (currentRoomStats.player_name.indexOf(otherUid) === 0) ? p1Data.current : p2Data.current;

                if (otherData.isGameOver || !otherUid) {
                    const myScore = g.score;
                    const oppScore = otherData.score;
                    if (myScore > oppScore) setWinner('WIN');
                    else if (myScore < oppScore) setWinner('LOSE');
                    else setWinner('TIE');
        ...
        const myNext = playerIndex === 0 ? nextCanvasRef : oppNextCanvasRef;
        drawNext(myNext, g.nextBlockType, g.level, 25);
    ...
    useEffect(() => {
        if (socket) {
            ...
            socket.on('in_room', (data) => {
                ...
                if (data.player_name[0] === '') p1Data.current = empty;
                if (data.player_name[1] === '') p2Data.current = empty;
                setUiStates({ p1: p1Data.current, p2: p2Data.current });
            });
            ...
            socket.on('tetris_client_block', (data) => {
                ...
                const nextCanvas = targetIdx === 0 ? nextCanvasRef : oppNextCanvasRef; drawNext(nextCanvas, data.nextBlockType, data.level, 25);
            });
            ...
            socket.on('client_gameover', (data) => {
                ...
                const amIPlayer = (myIdx >= 0 && myIdx < 2);

                const p1Dead = p1Data.current.isGameOver;
                const p2Dead = p2Data.current.isGameOver;
                const otherUid = currentRoomStats.player_name.find(name => name !== '' && currentRoomStats.player_name.indexOf(name) !== targetIdx);

                if ((p1Dead && p2Dead) || !otherUid) {
                    const s1 = p1Data.current.score;
                    const s2 = p2Data.current.score;
                    if (amIPlayer) {
                        ...
                        if (s1 > s2) setWinner('P1_WIN');
                        else if (s2 > s1) setWinner('P2_WIN');
                        else setWinner('TIE');
                    }
    ...
    useEffect(() => {
        if (gameState === 'GAMEOVER' && playerIndex !== -1 && playerIndex < 2) {
            const myScore = playerIndex === 0 ? p1Data.current.score : p2Data.current.score;
            ...
            const recordScore = async () => {
                try {
                    ...
                    console.log("Multiplayer score recorded:", myScore, winStatus);
    ...
    useEffect(() => { if (gameState === 'LOBBY' && playerIndex === 0 && roomStats.ready_level[0] !== -1 && roomStats.ready_level[1] !== -1) { const seeds = Array.from({ length: 1000 }, () => Math.floor(Math.random() * 1000)); socket.emit('send_random_block', { room_num: roomData.roomNum, random_block: seeds }); } }, [playerIndex, roomStats.ready_level, gameState]);
    useEffect(() => { if (gameState === 'COUNTING' && (roomStats.ready_level[0] === -1 || roomStats.ready_level[1] === -1)) { setGameState('LOBBY'); setPreGameCount(3); } }, [roomStats.ready_level, gameState]);
    ...
    const renderPlayerInfo = (idx) => {
        ...
        if (name === '') return <div style={{ width: '250px' }}></div>;
        ...
        return (
            <div style={{ width: '250px', textAlign: idx === 0 ? 'left' : 'right', fontFamily: '"Press Start 2P"' }}>
                ...
                <div style={{ display: 'flex', justifyContent: idx === 0 ? 'flex-start' : 'flex-end', gap: '15px', marginBottom: '10px' }}>
                    ...
                    <span style={{ color: 'yellow', fontSize: '0.9rem' }}>Tie</span>
                </div>
                <div style={{ display: 'flex', justifyContent: idx === 0 ? 'flex-start' : 'flex-end', gap: '35px', color: 'white', fontSize: '1.1rem', marginBottom: '30px' }}>
                    <span>{roomStats.player_win[idx] || 0}</span>
                    <span>{roomStats.player_lose[idx] || 0}</span>
                    <span>{roomStats.player_tie[idx] || 0}</span>
                </div>
                <div style={{ color: 'red', fontSize: '1.2rem', marginBottom: '15px' }}>Rank</div>
                <img src={getRankIcon(name, best, roomData.leaderboard)} alt="rank" style={{ width: '70px', marginBottom: '30px' }} />
    ...
    const renderLobby = () => { const bothReady = roomStats.ready_level[0] !== -1 && roomStats.ready_level[1] !== -1; return (<div className="lobby-overlay" style={{ position: 'absolute', inset: 0, background: '#000', zIndex: 2000, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px', color: 'white', fontFamily: '"Press Start 2P"' }}> <div style={{ position: 'relative', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '40px' }}> <h1 style={{ fontSize: '4rem', color: roomStats.player_name[1] ? '#0ff' : '#f00', margin: '0 0 20px 0' }}>TETRIS</h1> <h2 style={{ fontSize: '2.5rem', color: 'white', margin: 0 }}>LEVEL</h2> <div style={{ position: 'absolute', right: '20px', top: '20px', color: 'red', fontSize: '1.1rem' }}>Room ID : {roomData.roomNum}</div> </div> <div style={{ display: 'flex', justifyContent: 'space-between', width: '1200px', flexGrow: 1 }}> {renderPlayerInfo(0)} <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '600px', marginTop: '40px' }}> {bothReady && <div style={{ color: 'red', fontSize: '0.9rem', marginBottom: '30px', textAlign: 'center', lineHeight: '1.5' }}>Both player selected, game will start in 3 seconds...</div>} <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 80px)', gap: '40px 20px', textAlign: 'center', fontSize: '1.8rem' }}> {[...Array(20).keys(), "", "", 29].map((lv, idx) => <div key={idx} onClick={() => lv !== "" && playerIndex !== -1 && playerIndex < 2 && roomStats.ready_level[playerIndex] === -1 && setSelectedLevel(lv)} style={{ cursor: lv !== "" ? 'pointer' : 'default', color: selectedLevel === lv ? 'cyan' : 'white' }}>{lv}</div>)} </div> <div style={{ marginTop: 'auto', marginBottom: '40px', width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 50px' }}> <div onClick={onBack} style={{ cursor: 'pointer', fontSize: '1.5rem', color: 'white', width: '300px', textAlign: 'left' }}>back</div> <div style={{ fontSize: '1rem', color: 'yellow', width: '300px', textAlign: 'right' }}>Play Mode : <span style={{ color: 'white' }}>{playerIndex < 2 ? `Player${playerIndex + 1}` : 'Spectator'}</span></div> </div> {playerIndex !== -1 && playerIndex < 2 && <button onClick={roomStats.ready_level[playerIndex] === -1 ? handleReady : handleCancelReady} style={{ background: 'none', border: '3px solid white', color: roomStats.ready_level[playerIndex] === -1 ? 'white' : '#ff4444', padding: '10px 40px', fontSize: '1.5rem', fontFamily: '"Press Start 2P"', cursor: 'pointer', marginTop: '20px', minWidth: '280px' }}>{roomStats.ready_level[playerIndex] === -1 ? 'SELECTED' : 'CANCEL'}</button>} </div> {renderPlayerInfo(1)} </div> </div>); };
        ...
        return (
            <div style={{ display: 'flex', alignItems: 'center', gap: '40px' }}>
                ...
                <div className="right-stats" style={{ width: '160px', display: 'flex', flexDirection: 'column', gap: '30px', fontFamily: '"Press Start 2P"', color: 'white' }}>
                    ...
                    <div style={{ fontSize: '1rem', color: 'white', marginTop: '10px' }}>{data.hz} HZ</div>
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="game-screen" ref={gameScreenRef}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '80px', position: 'relative', zIndex: 1, padding: '20px' }}>
                {renderPlayerModule(0)} {renderPlayerModule(1)}
            </div>
            {gameState === 'GAMEOVER' && (
